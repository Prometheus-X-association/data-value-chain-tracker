  ...
  try {    
    ...
    body.participantShare.forEach((participant) =>{
      if(participant.rewardDepositor){
        ...
        participant.participantWallet = addresses[body.participantShare.indexOf(participant)]
      }
    ...
    const command = `npx ts-node -P ./tsconfig.json './e2e-tests/integration/incenti-trace.ts'`;
